...
export default function ConfigurationPanel({ apiBaseUrl }: { apiBaseUrl: string }) {
  ...
  switch (type) {
    case 'Avatar':
      return <AvatarSidebarPanel key={selectedBlockId} data={data} setData={(data) => setBlock({ type, data })} apiBaseUrl={apiBaseUrl} />;
    case 'Button':
    ...
    case 'Image':
      return <ImageSidebarPanel key={selectedBlockId} data={data} setData={(data) => setBlock({ type, data })} apiBaseUrl={apiBaseUrl} />;
    case 'EmailLayout':
